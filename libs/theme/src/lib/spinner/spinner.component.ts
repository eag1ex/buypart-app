...
export class SpinnerComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes.parentElement) {
     ...
     if (!(this.parentElement.nativeElement.classList.value || '').includes('blur-app-while-loading')){
        this.parentElement.nativeElement.classList.add('blur-app-while-loading');
     }
    }

    if (this.loaded) {
      if (!(this.elementRef.nativeElement.classList.value || '').includes('hide-spinner')){
